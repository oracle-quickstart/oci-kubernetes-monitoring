...
static const struct argp_option opts[] = {
	...
	{ "count", 'c', NULL, 0, "Count connects per src ip and dst ip/port" },
        { "extended-count", 'e', NULL, 0, "Count the connects per command, src ip, dst ip/port and prints the output (space separated, without headers) to STDOUT periodically (default 60s)" },
        { "interval", 'i', "INTERVAL", 0, "Interval for extended-count in seconds, defaults to 60" },
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'c':
		...
		break;
	case 'i':
		err = get_int(arg, &env.interval, 1, INT_MAX);
		if (err) {  
...
static void print_count_ipv4(int map_fd, time_t start)
{
...
static void print_count_ipv6(int map_fd, time_t start)
{
...
static void print_count(int map_fd_ipv4, int map_fd_ipv6)
{
	...
	print_count_header();
	print_count_ipv4(map_fd_ipv4, 0);
	print_count_ipv6(map_fd_ipv6, 0);
}

static void print_extended_count(int map_fd_ipv4, int map_fd_ipv6)
{
        time_t end;
		time_t start = 0;
        while (!exiting) {
		start = time(NULL);
		end = time(NULL) + env.interval;
    		while (time(NULL) <= end && !exiting) {
    			sleep(1);
		}
		print_count_ipv4(map_fd_ipv4, start);
		print_count_ipv6(map_fd_ipv6, start);
	}
...
int main(int argc, char **argv)
{
	...
	// ec takes precedence over count
	if (env.ec)
		obj->rodata->do_ec = true;
	if (env.count)
	...
	// count per unique source_port not applicable for extended count
	if (env.source_port && !env.ec)
	...
	// ec takes precedence over count
        if (env.ec) {      
		print_extended_count(bpf_map__fd(obj->maps.ipv4_count), 
