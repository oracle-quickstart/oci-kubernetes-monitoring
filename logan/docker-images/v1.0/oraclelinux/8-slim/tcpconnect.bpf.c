...
static  __always_inline void extended_count_v4(struct sock *sk, __u16 sport, __u16 dport)
{
        struct ipv4_flow_key key = {};
        static __u64 zero;
        __u64 *val;
...
static __always_inline void extended_count_v6(struct sock *sk, __u16 sport, __u16 dport)
{
        struct ipv6_flow_key key = {};
        static const __u64 zero;
        __u64 *val;
...
exit_tcp_connect(struct pt_regs *ctx, int ret, int ip_ver)
{
	...
	if (do_ec) {
		if (ip_ver == 4)
			extended_count_v4(sk, sport, dport);
		else
			extended_count_v6(sk, sport, dport);
	} else if (do_count) {
		if (ip_ver == 4)
