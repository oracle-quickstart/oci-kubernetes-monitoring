locals {
  ...
  dynamic_group_already_exists = length(data.oci_identity_dynamic_groups.oke_dynamic_group.dynamic_groups) == 0 ? false : true
  ...
  oke_monitoring_policy_already_exists = length(data.oci_identity_policies.oke_monitoring_policy.policies) == 0 ? false : true
}
...
resource "oci_identity_dynamic_group" "oke_dynamic_group" {
  ...
  count = var.opt_create_dynamicGroup_and_policies && ! local.dynamic_group_already_exists ? 1 : 0
}
...
resource "oci_identity_policy" "oke_monitoring_policy" {
  ...
  count      = var.opt_create_dynamicGroup_and_policies && ! local.oke_monitoring_policy_already_exists ? 1 : 0
