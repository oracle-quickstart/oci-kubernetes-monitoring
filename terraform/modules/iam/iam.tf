...
locals {
  ...
  mgmt_agent_agent_policy    = ["Allow dynamic-group ${local.dynamic_group_name} to use METRICS in ${local.policy_scope} WHERE target.metrics.namespace = 'mgmtagent_kubernetes_metrics'"]
  ...
  policy_statements    = concat(local.fluentd_agent_policy, local.mgmt_agent_agent_policy)
}
