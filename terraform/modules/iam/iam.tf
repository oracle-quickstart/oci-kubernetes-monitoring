...
locals {
  ...
  policy_stmts = {
    ...
    infra_discovery_stmt = [
      ...
      "Allow service loganalytics to {VCN_READ,SUBNET_READ,LOAD_BALANCER_READ} in ${local.oke_compartment_scope}",
      ...
      "Allow dynamic-group ${local.dynamic_group_name} to read clusters in tenancy where target.cluster.id=${var.oke_cluster_ocid}",
    ...
    service_discovery_stmt = var.create_service_discovery_policies ? distinct([
      ...
      # Required to create enable logging
      ...
      # Required configure service connector
