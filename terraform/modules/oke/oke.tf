locals {
  ...
  # cluster_private_ip_port = var.oke_is_private ? local.cluster_data.endpoints[0].private_endpoint : null
  ...
  name             = local.cluster_data.name
  time_created     = local.cluster_data.metadata[0].time_created # "2021-05-21 16:20:30 +0000 UTC"
  is_private       = !local.cluster_data.endpoint_config[0].is_public_ip_enabled
  ...
  # oke_cluster_name = var.oke_cluster_name == "DEFAULT" ? local.cluster_data_name : var.oke_cluster_name
  # oke_vcn_id       = var.oke_is_private ? local.cluster_data.vcn_id : null
