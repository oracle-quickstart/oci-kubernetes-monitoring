...
locals {
  ...
  create_new_logGroup   = var.new_logGroup_name != null
  create_new_k8s_entity = var.new_oke_entity_name != null
}
...
resource "oci_log_analytics_log_analytics_entity" "oke" {
  ...
  name             = var.new_oke_entity_name #TODO Decide on expected name
  namespace        = local.oci_la_namespace
  #Optional
